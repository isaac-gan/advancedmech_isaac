...
void dayOfTheWeek(unsigned char position, char* day){
    ...
    int i = 0;
    while(DAYOFTHEWEEK[position][i]!='.'){ //leave as a dot at the end so it knows when to stop copying char over
        day[i] = DAYOFTHEWEEK[position][i];
        i++;
    }
    day[i] = '\0';//adds null char at end so sprintf knows to stop adding char when we take the week and use a sprintf %s to generate the name of the day of the week
}

//when call below fxn will send a date and time
...
rtccTime readRTCC() {//return something of structure rtccTime. Read the time and print it onto display
    rtccTime time;
    
    //Store into respective fields for time
    ...
    //Store into respective fields for date
